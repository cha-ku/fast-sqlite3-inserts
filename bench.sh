...
echo "$(date)" "[PYTHON] running naive.py (1_000_000) inserts"
time python3 naive.py
# lets verify the data exists
if [[ $(sqlite3 naive.db  "select count(*) from user";) != 1000000 ]]; then
  echo "data verification failed"
fi

rm -rf cppnaive.db cppnaive.db-shm cppnaive.db-wal
echo "$(date)" "[C++] running naive.cpp (1_000_000) inserts"
g++ naive.cpp -lsqlite3
time ./a.out
# lets verify the data exists
if [[ $(sqlite3 cppnaive.db  "select count(*) from user";) != 1000000 ]]; then
  echo "data verification failed"
fi
